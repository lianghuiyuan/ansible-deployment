...
ifeq ($(USE_DOCKER),1)
	PLAYBOOK=docker run --rm -it -v $(WORK_PATH):/work -e ANSIBLE_HOST_KEY_CHECKING=False --network host -w /work/ansible registry.jetio.net/library/ansible:2.10-$(CPU_ARCH) ansible-playbook
else
